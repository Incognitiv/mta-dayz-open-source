...
function getWeaponDamage(weapon)
  for i, weapon2 in ipairs(damageTable) do
    ...
    if weapon1 == weapon then
      if getElementData(getLocalPlayer(), "humanity") == 5000 and (weapon2[1] == "Pistol" or weapon2[1] == "Silenced Pistol" or weapon2[1] == "Uzi") then
...
function playerGetDamageDayZ(attacker, weapon, bodypart, loss)
  ...
  if getElementData(attacker, "zombie") then
    setElementData(getLocalPlayer(), "blood", getElementData(getLocalPlayer(), "blood") - math.random(100, 600))
...
function showPlayerDamageScreen(visibly2, stateControle2)
  ...
  if visibly >= 0.75 and stateControle == "up" then
    stateControle = "down"
  end
  if visibly < 0 then
    guiSetVisible(whiteWindow, false)
...
function playerPingCheck()
  if getPlayerPing(getLocalPlayer()) > 600 then
    pingFails = pingFails + 1
    if pingFails == 5 then
      triggerServerEvent("kickPlayerOnHighPing", getLocalPlayer())
      return
    end
    startRollMessage2("Ping", "Your Ping is over 600! (" .. pingFails .. "/5)", 255, 22, 0)
